...
const Login = () => {
  ...
  const onLogInClick = async () => {
    try {
      ...
      fetchLogin(auth.currentUser.displayName, auth.currentUser.email, auth.currentUser.uid);
    } catch (error) {
      ...
      {emailVerified && codeCheck && (
        <UnivCertify className="code">
          ...
          const OptionButton = styled.button`
            ...
            padding: 12px;
            margin: 0 5px;
            background-color: ${({ selected }) => (selected ? "#5e5ce6" : "#f5f5fa")}; /* Updated background color based on selection */
            color: ${({ selected }) => (selected ? "white" : "#000")}; /* Color based on selection */
            border: none; /* Removed border */
